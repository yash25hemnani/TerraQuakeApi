	...
	return (
		<header className="fixed top-0 left-0 w-full box-border backdrop-blur-2xl bg-opacity-60 text-white shadow-lg py-4 px-4 flex items-center justify-between lg:justify-around z-50">
			...
			<div className="flex items-center text-2xl font-bold w-fit h-12 relative">
				<img src={Sismic} alt="Logo" className="absolute z-20 w-36 h-20 opacity-40 object-contain" />
			...
			<div className="hidden lg:flex items-center gap-4 text-[14px] lg:text-[16px]">
				{isLoggedIn ? (
					<>
						...
						<button onClick={handleLogout} className="font-semibold border-purple-500 hover:border-b-2 hover:text-purple-400 cursor-pointer">
							Logout
						</button>
					</>
				) : (
			...
			{/* Mobile Dropdown */}
			<div className={`lg:hidden absolute top-full w-[95%] left-[2.5%] bg-[#090414] text-white overflow-hidden transition-all duration-500 ease-in-out z-40 rounded-b-xl ${isOpen ? "max-h-[500px] opacity-100 pointer-events-auto py-6 px-4" : "max-h-0 opacity-0 pointer-events-none py-0 px-4"}`}>
				{/* Mobile Links */}
				<div className="flex flex-col items-center gap-4 text-xl mb-4">
					{listItems.map((item) => (
						<NavLink key={item.name} to={item.path} onClick={() => setIsOpen(false)} className={({ isActive }) => `hover:text-purple-400 w-full text-center ${isActive ? "text-purple-400 font-semibold border-b-2 border-purple-500" : "text-gray-300"}`}>
							{item.name}
						</NavLink>
					))}
				</div>
