...
export const resetPassword = async (req, res) => {
	try {
		...
		const user = await User.findOne({ email: req.email }).select("password -_id");

		const userPasswordFromDb = user.password;

		console.log(userPasswordFromDb);

		const passwordEquality = await compare(newUserPassword, userPasswordFromDb);
		...
		handleHttpError(res, "New password cannot be the same as the old password !", 409);
	}
